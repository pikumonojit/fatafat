...
export default function AdminPage() {
  ...
  async function publish() {
    ...
    const payload = rows.filter((r) => r.result.trim()).map((r) => ({ result_date: date, round_number: r.number, round_name: r.name, result_time: r.result_time || '', result: r.result.trim(), status: 'published', published_at: new Date().toISOString() }))
