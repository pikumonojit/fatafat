...
export default async function Home() {
  const { data: results } = await supabase.from('results').select('*').eq('status', 'published').order('result_date', { ascending: false }).order('round_number')
  ...
  return <main className="page">
    ...
    <section id="today" className="card"><div className="cardhead"><h2>Today's Results</h2><span className="live">● PUBLISHED</span></div><div className="grid">{todayResults.length ? todayResults.map(r => <div className="result" key={r.id}><div><b>{r.round_name}</b><small>{r.result_time}</small></div><strong>{r.result || '—'}</strong></div>) : <div className="empty">No results published yet.</div>}</div></section>
